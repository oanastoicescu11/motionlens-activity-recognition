...
_STAIRS_MOVING_GUARD_MIN_WALKRUN_MASS = 0.30
_STAIRS_MOVING_GUARD_MAX_PROBA = 0.15
...
_LOCOMOTION_TRANSITIONS_ENERGY_MIN = 8.0   # m/s²: below this, transitions may be genuine
...
_STAIRS_DEMOTE_WALKING_CADENCE_MIN = 90.0
_STAIRS_DEMOTE_WALKING_CADENCE_MAX = 130.0
_STAIRS_DEMOTE_WALKING_PERIODICITY_MIN = 0.55
...
def apply_locomotion_transitions_guard(
    ...
    However, if the previous decoded state was static (sit/lay or stand), the current window
    is likely the start of a genuine postural transition (e.g. sit→stand), so we skip demotion
    and let the HMM handle it naturally.
    ...
    # Skip demotion if previous decoded state was static — likely a genuine postural transition
    if previous_state is not None:
        ...
        if previous_posterior is not None:
            prev_idx = int(np.argmax(previous_posterior))
            prev_label = label_order[prev_idx]
            if prev_label in STATIC_LABELS:
                return emission, False, "none"
    ...
    *,
    ...
    if pred_label == "run":
        ...
        if (
            ...
            _STAIRS_DEMOTE_WALKING_CADENCE_MIN <= cadence_spm <= _STAIRS_DEMOTE_WALKING_CADENCE_MAX
            and periodicity_strength >= _STAIRS_DEMOTE_WALKING_PERIODICITY_MIN
        ):
            # Walking cadence with strong periodicity → likely walking, not stairs
            should_downgrade = True
            reason = "stairs-to-walk-walking-cadence"
