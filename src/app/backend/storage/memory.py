...
class InMemoryStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._session_summary: dict[str, dict[str, Any]] = {}
    ...
    def create_session(self, meta: SessionMeta) -> None:
        ...
        self._history[meta.session_id] = deque(maxlen=self._max_history_entries)
    ...
    def finalize_session(self, session_id: str, *, finalized_at_ns: int) -> SessionMeta | None:
        ...
        if meta is None:
            return None
        meta.finalized_at_ns = int(finalized_at_ns)
        return meta
    ...
    def append_inference_history(self, session_id: str, snapshot: Mapping[str, Any]) -> None:
        ...
        if session_id not in self._history:
            self._history[session_id] = deque(maxlen=self._max_history_entries)
        self._history[session_id].append(dict(snapshot))

    def read_inference_history(self, session_id: str, limit: int = 0) -> list[dict[str, Any]]:
        values = self._history.get(session_id)
        if not values:
            return []
        selected = list(values) if limit <= 0 else list(values)[-limit:]
    ...
    def set_session_summary(self, session_id: str, summary: Mapping[str, Any] | None) -> None:
        if summary is None:
            ...
            return
        self._session_summary[session_id] = dict(summary)

    def get_session_summary(self, session_id: str) -> dict[str, Any] | None:
        value = self._session_summary.get(session_id)
