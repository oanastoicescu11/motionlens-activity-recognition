...
class RedisStore:
    ...
    def _history_key(self, session_id: str) -> str:
        return self._key(f"history:{session_id}")

    def _summary_key(self, session_id: str) -> str:
        return self._key(f"summary:{session_id}")
    ...
    def _decode_session_meta(self, payload: dict[str, str] | None) -> SessionMeta | None:
        ...
        try:
            return SessionMeta(
                ...
                finalized_at_ns=int(payload.get("finalized_at_ns", "0")),
            )
        except Exception:
            LOGGER.warning("Failed to decode Redis session metadata payload", exc_info=True)
            return None
    ...
    def create_session(self, meta: SessionMeta) -> None:
        ...
        payload = {
            ...
            "finalized_at_ns": str(meta.finalized_at_ns),
        }
    ...
    def finalize_session(self, session_id: str, *, finalized_at_ns: int) -> SessionMeta | None:
        ...
        self._client.hset(
            self._session_key(session_id),
            mapping={"finalized_at_ns": str(int(finalized_at_ns))},
        )
        self._client.expire(self._session_key(session_id), ttl)
        return self._get_session_meta(session_id)
    ...
    def read_raw_points(
        ...
        for raw in raw_values:
            try:
                ...
                LOGGER.warning("Skipping malformed raw point payload in Redis for session_id=%s", session_id, exc_info=True)
                continue
        return out
    ...
    def pop_inference_task(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            LOGGER.warning("Skipping malformed inference queue payload from Redis", exc_info=True)
            return None
    ...
    def get_latest_inference(self, session_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            LOGGER.warning("Failed to decode latest inference snapshot from Redis for session_id=%s", session_id, exc_info=True)
            return None
        return parsed if isinstance(parsed, dict) else None
    ...
    def append_inference_history(self, session_id: str, snapshot: Mapping[str, Any]) -> None:
        ...
        self._client.rpush(key, json.dumps(dict(snapshot)))
        self._client.ltrim(key, -self._max_history_entries, -1)
        self._client.expire(key, self._session_ttl_seconds(meta.expires_at_ns))
    ...
    def read_inference_history(self, session_id: str, limit: int = 0) -> list[dict[str, Any]]:
        ...
        for raw in raw_values:
            try:
                ...
                LOGGER.warning("Skipping malformed inference history payload in Redis for session_id=%s", session_id, exc_info=True)
                continue
    ...
    def get_session_summary(self, session_id: str) -> dict[str, Any] | None:
        raw = self._client.get(self._summary_key(session_id))
        if raw is None:
            return None
        try:
            parsed = json.loads(raw)
        except Exception:
            LOGGER.warning("Failed to decode finalized session summary from Redis for session_id=%s", session_id, exc_info=True)
            return None
        return parsed if isinstance(parsed, dict) else None
    ...
    def get_decoder_state(self, session_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            LOGGER.warning("Failed to decode decoder state from Redis for session_id=%s", session_id, exc_info=True)
            return None
        return parsed if isinstance(parsed, dict) else None
