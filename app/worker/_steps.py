...
from core.features import compute_window_features
...
def _load_model() -> tuple[InferenceArtifacts, dict[str, Any]]:
    """Load pocket_only inference bundle from artifacts/model/.
    ...
    Falls back to the raw model without a decoder if the bundle is absent.
    """
    artifacts_dir = Path(__file__).resolve().parents[2] / "artifacts" / "model"
    bundle_path = artifacts_dir / "inference_bundle.joblib"
    raw_path = artifacts_dir / "model_pocket_only.joblib"
    ...
    if bundle_path.exists():
        ...
        LOGGER.warning(
            "inference_bundle.joblib not found at %s; loading raw model without decoder or specialist.",
            artifacts_dir,
        )
        bundle = {
            "base_model": joblib.load(raw_path),
            "label_order": ["walk", "run", "stairs", "still", "lay", "transitions", "locomotion-other"],
            "temporal_decoder": "none",
            "log_init_probs": None,
            "log_trans_probs": None,
        ...
        raise FileNotFoundError(
            ...
            f"Run: python processing/train_pocket_only_v1.py and copy output to artifacts/model/"
        )
    ...
    meta = {
        ...
        "live_decoder_reset_mode": "session",
        "live_sequence_boundary_gap_seconds": 3.0,
...
def _resolve_placement_label(task: dict[str, Any], bundle_meta: dict[str, Any]) -> tuple[str, str]:
    """Pocket_only mode: always use front_pocket (no user placement selection)."""
    return "front_pocket", "pocket_only"
